...
def analyze_ticker(ticker, df):
    try:
        ...
        ema20, ema50, ema200 = df["Close"].ewm(span=20).mean().iloc[-1], df["Close"].ewm(span=50).mean().iloc[-1], df["Close"].ewm(span=200).mean().iloc[-1]
        ...
        score, reasons = 0, []
        if ema20 > ema50 > ema200: score += 15; reasons.append("Bullish EMAs")
        if bos: score += 20; reasons.append("BOS Breakout")
        if sweep: score += 15; reasons.append("Liquidity Sweep")
        if rsi_v < 40: score += 10; reasons.append("RSI Oversold")
        if adx_v > 25: score += 10; reasons.append("Strong Trend")
        if cmf_v > 0: score += 10; reasons.append("Money Inflow")
        if obv_trend > 0: score += 10; reasons.append("OBV Rising")
        if vol > avg_vol * 1.5: score += 10; reasons.append("Volume Surge")
...
def run_scan(tickers):
    session = Session()
    session.headers.update({'User-Agent': 'Mozilla/5.0'}) # Fixed 401 Unauthorized

    print(f"--- Downloading data for {len(tickers)} tickers ---")
    data = yf.download(tickers, period="1y", group_by="ticker", progress=False, session=session)
...
if __name__ == "__main__":
    ticker_list = [
        "AAPL", "MSFT", "NVDA", "GOOGL", "AMZN", "META", "TSLA", "AMD", "NFLX", "BLOCK",
    ...
    start = time.time()
    top_picks = run_scan(ticker_list)

    print("\n🔥 TOP 10 SMART MONEY PICKS")
    for r in top_picks:
        print(f"\n{r.ticker} [{r.signal}] Score: {r.score}% | Conf: {r.confidence}%")
        print(f"  Entry: ${r.price} | SL: ${r.stop_loss} | Targets: ${r.target_1}, ${r.target_2}")
        print(f"  Setup: {r.setup_type} | Reasons: {', '.join(r.reasons)}")

    print(f"\n⏱ Scan completed in {round(time.time()-start,2)} seconds")
